...
# ---- safe concat helper ----
# Garante que listas com None/[] não quebrem em pd.concat
# e sempre retorna um DataFrame válido
import pandas as pd  # ensure pandas is available in this scope

def _concat_safely(frames):
    try:
...
origins_env = os.getenv("FRONTEND_ORIGIN", "http://localhost:5173,http://127.0.0.1:5173,http://localhost:3000,http://127.0.0.1:3000")
origin_list = [o.strip() for o in origins_env.split(",") if o.strip()]
origin_regex = os.getenv("FRONTEND_ORIGIN_REGEX", "")



# Adicionar suporte a Netlify preview deploys automaticamente
if not origin_regex:
    origin_list.extend([
        "https://*.netlify.app",
        "https://*.netlify.live"
...
print(f"[CORS] Allowing origins: {origin_list}")
print(f"[CORS] Origin regex: {origin_regex if origin_regex else 'Not set'}")
if origin_regex:
    app.add_middleware(
        CORSMiddleware,
...
try:
    ...
    if GEMINI_API_KEY:
        ...
        print(f"[AI] Gemini model loaded: {GEMINI_MODEL}")
except Exception as e:
    # Mantém gemini_model=None -> o código cai no fallback sem IA
    print(f"[AI] Gemini not available: {e}")
    gemini_model = None
...
def api_summary(client: str, ym: str = Query(...), embarcador: str = Query(...)):
    ...
    booking_concat = _concat_safely(booking_frames)
    multi_concat = _concat_safely(multi_frames)
    transp_concat = _concat_safely(transp_frames)
